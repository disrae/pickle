...
// Get a single builder chat by ID
...
export const create = mutation({
    ...
    handler: async (ctx, { title, description }) => {
        ...
        const now = Date.now();
        const chatId = await ctx.db.insert("builderChats", {
            title,
            description,
            createdBy: userId,
            createdAt: now,
...
export const addParticipant = mutation({
    ...
    handler: async (ctx, { chatId }) => {
        ...
        const participantId = await ctx.db.insert("builderChatParticipants", {
            chatId,
            userId,
            joinedAt: Date.now(),
        });
...
// Search builder chats
export const search = query({
    args: { searchTerm: v.string() },
    handler: async (ctx, { searchTerm }) => {
        const allChats = await ctx.db.query("builderChats").collect();
...
// Toggle builder chat notification preference
...
// Delete a builder chat (admin only)
