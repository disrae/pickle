...
export default function BuilderChatsListScreen() {
    const { top } = useSafeAreaInsets();
    const router = useRouter();
    const [searchQuery, setSearchQuery] = useState("");
    const [showNewChatModal, setShowNewChatModal] = useState(false);

    const allChats = useQuery(api.builderChats.list);
    const searchResults = useQuery(
        api.builderChats.search,
        searchQuery.trim() ? { searchTerm: searchQuery } : "skip"
    );

    const createChat = useMutation(api.builderChats.create);
    ...
    const handleCreateChat = async (title: string, description?: string) => {
        ...
        router.push(`/builder/chats/${chatId}`);
    };
                        ...
                        <View className="bg-white/95 rounded-xl overflow-hidden">
                            {chats.map((chat, index) => (
                                <View key={chat._id}>
                                    <TouchableOpacity
                                        onPress={() =>
                                            router.push(`/builder/chats/${chat._id}`)
                                        }
